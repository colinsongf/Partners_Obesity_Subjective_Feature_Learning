...
select 
	...
	sum(Count) as Count
from sentimentparagraph
group by ID, Sentiment
order by ID
"""
...
def getAndBuildSentiment(connection, trainDomain, testDomain):
	...
	for id in trainDomain:
		...
		for sentLabel in sentimentDomain:
			tempList.append(sentLabel)

			if returnHash.has_key(id) and returnHash[id].has_key(sentLabel):
				tempList.append(returnHash[id][sentLabel])
			else:
				tempList.append(0)
...
def main():
	# mysql connection
	featureSelectionName = "simple sentiment, no weak/strong"
